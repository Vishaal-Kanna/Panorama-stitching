...
def transformer(U, theta, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _interpolate(im, x, y, out_size, scale_h):
        with tf.compat.v1.variable_scope('_interpolate'):
            ...
            # channels dim
            im_flat = tf.reshape(im, tf.stack([num_batch*height*width, channels]))
    ...
    def _meshgrid(height, width, scale_h):
        with tf.compat.v1.variable_scope('_meshgrid'):
            ...
            if scale_h:
                ...
                x_t = tf.matmul(tf.ones(shape=tf.stack([height, 1])),
                                tf.transpose(tf.expand_dims(tf.linspace(0.0, tf.cast(width, 'float32'), width), 1),
                                             [1, 0]))
                y_t = tf.matmul(tf.expand_dims(tf.linspace(0.0, tf.cast(height, 'float32'), height), 1),
                                tf.ones(shape=tf.stack([1, width])))
    ...
    def _transform(theta, input_dim, out_size, scale_h):
        with tf.compat.v1.variable_scope('_transform'):
            ...
            grid = tf.reshape(grid, tf.stack([num_batch, 3, height*width]))
